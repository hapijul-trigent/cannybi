...
#             assistant_response = (
#                 "I can only answer database-related queries based on the schema.\n\n"
#                 "**For example, you can ask:**\n"
#                 "- *'What were the total sales in the last quarter?'*\n"
#                 "- *'How many customers placed an order last month?'*\n"
#                 "- *'What is the refund rate for product X?'*\n\n"
#                 "Please provide a structured query related to the database."
#             )
#         else:
#             chart_code = None
#             query_results = {}
...
#                     st.toast(f"Something went wrong while executing queries: {e}")
#                     st.toast("Don't worry auto-Fixing query. Hold-on tight!")
#                     with st.spinner("Auto-Fixing Queries..."):
...
#             except Exception as e:
#                 st.toast(f"Please refresh and retry: {e}")

#             if not chart_code or chart_code.strip() == "None":
#                 st.warning("No chart data available for visualization.")
#             else:
#                 try:
#                     exec_globals = {}
#                     exec(chart_code.replace("```python", "").replace("```", ""), {
#                         "plt": plt, "sns": sns, "pd": pd, "st": st, "np": np, "os": os
#                     }, exec_globals)

#                 except Exception as e:
#                     st.toast(f"No Visualization available: {e}")
...
#                 except Exception as e:
#                     st.toast(f"Please refresh and try again: {e}")
            ...
            if intent_analysis['intent'].lower() == 'misleading_query':
                ...
                if chart_code:
                    try:
                        ...
                        st.toast(f"Error in visualization: {e}")
